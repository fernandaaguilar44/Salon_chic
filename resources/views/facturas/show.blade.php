...
<head>
    ...
    <style>
        ...
        .invoice-table tbody tr { transition: all 0.3s ease; }
        .invoice-table tbody tr:hover { background-color: rgba(228, 0, 124, 0.05); transform: scale(1.005); }
        ...
        .totals-table th, .totals-table td { padding: 12px; border-color: rgba(228, 0, 124, 0.1); }
        .totals-table th { background: rgba(123, 42, 141, 0.1); color: #7B2A8D; font-weight: 600; }
        .totals-table td { background: rgba(255, 255, 255, 0.9); color: #333; font-weight: 500; }
    ...
    <div class="invoice-container">
        ...
        <div class="invoice-header">
            ...
            <div class="invoice-meta">
                ...
                <div class="details-grid" style="text-align: right;">
                    <div><strong>Fecha:</strong> {{ \Carbon\Carbon::parse($factura->fecha)->format('d/m/Y') }}</div>
        ...
        <div class="table-responsive">
            <table class="table invoice-table">
                ...
                @forelse($factura->detalles as $detalle)
                    <tr>
                        <td>{{ $detalle->producto->nombre ?? $detalle->nombre_producto_manual }}</td>
                        <td>{{ $detalle->cantidad }}</td>
                        <td>L {{ number_format($detalle->precio_unitario, 2) }}</td>
                        <td>L {{ number_format($detalle->producto->precio_venta ?? $detalle->precio_unitario, 2) }}</td>
        ...
        <div class="totals-container">
            <table class="table totals-table">
                ...
                <tr>
                    <th>IVA (15%)</th>
                    <td>L {{ number_format($factura->detalles->where('tipo_impuesto','gravado15')->sum('subtotal') * 0.15, 2) }}</td>
                </tr>
                <tr>
                    <th>EXONERADO</th>
                    <td>L {{ number_format($factura->importe_exonerado ?? 0, 2) }}</td>
                </tr>
                <tr>
                    <th>EXENTO</th>
                    <td>L {{ number_format($factura->importe_exento ?? 0, 2) }}</td>
                </tr>
                <tr>
                    <th>TOTAL COMPRA</th>
