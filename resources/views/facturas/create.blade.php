...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const modalDetallesProducto = document = document.getElementById('modalDetallesProducto');
            ...
            const actualizarTotales = () => {
                ...
                itemInputs.forEach(input => {
                    ...
                    const precio = parseFloat(document.querySelector(`input[name="items[${itemIndex}][precio_compra]"]`).value) || 0;
            ...
            const addProductRow = (productId, productName, purchasePrice, salePrice, productQuantity, taxType, forceNew = false) => {
                if (!productId || !purchasePrice || !productQuantity) {
                ...
                const productSubtotal = purchasePrice * productQuantity;
    ...
    `;
        ...
        <input type="hidden" name="items[${itemIndex}][precio_compra]" value="${purchasePrice.toFixed(2)}" data-item-index="${itemIndex}">
        <input type="hidden" name="items[${itemIndex}][precio_venta]" value="${salePrice.toFixed(2)}" data-item-index="${itemIndex}">
    `;
        ...
        `;
            ...
            btnAgregarConfirmar.addEventListener('click', function() {
                ...
                // --- CÓDIGO CORREGIDO PARA EL FORMATO DE NÚMEROS ---
                // Reemplaza la coma por el punto antes de convertir a número
                const precioCompra = parseFloat(precioCompraInput.value.replace(',', '.'));
                const precioVenta = parseFloat(precioVentaInput.value.replace(',', '.'));
                const cantidad = parseInt(cantidadInput.value.replace(',', '.'), 10);
                // --- FIN DEL CÓDIGO CORREGIDO ---

                const tipoImpuesto = document.querySelector(`[data-product-id="${productId}"]`).dataset.impuesto;

                const agregado = addProductRow(productId, nombre, precioCompra, precioVenta, cantidad, tipoImpuesto, false);
                if (agregado) {
            ...
            form.addEventListener('submit', async function(e) {
                ...
                if (!esNumeroUnico) {
                    numeroFacturaDisplay.classList.add('is-invalid');
                    numeroFacturaDisplay.insertAdjacentHTML('afterend', '<div class="invalid-feedback d-block">El número de factura ya ha sido registrado.</div>');
                    error = true;
                }
                ...
                if (filas.length === 0) {
                    ...
                    filas.forEach(cantidadInput => {
                        ...
                        const precioInput = itemsContainer.querySelector(`input[name="items[${index}][precio_compra]"]`);

                        // --- VALIDACIÓN FINAL - LOS VALORES YA VIENEN CORRECTOS ---
                        // La validación ahora solo revisa si son números válidos y positivos
            ...
            btnLimpiarFactura.addEventListener('click', function() {
                ...
                limpiarModalProductos();
                // Asegura que el campo de fecha se vuelva a llenar al limpiar
            ...
            oldItems.forEach(item => {
                ...
                const purchasePrice = parseFloat(item.precio_compra) || 0;
                const salePrice = parseFloat(item.precio_venta) || 0;
                const productQuantity = parseInt(item.cantidad) || 0;
                const taxType = item.tipo_impuesto ?? 'gravado15';

                if (productId && purchasePrice > 0 && productQuantity > 0) {
                    addProductRow(productId, productName, purchasePrice, salePrice, productQuantity, taxType, true);
                }
            });
            @endif
