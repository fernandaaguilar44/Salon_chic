...
<div class="invoice-container">
    ...
    <div class="invoice-header">
        <div class="company-info">
            ...
            <div class="details-grid">
                {{-- Asumiendo que $venta es la variable pasada al controlador --}}
                <div><strong>Nombre:</strong> {{ $venta->cliente->nombre_empresa }}</div>
            </div>
        </div>
    ...
    <div class="client-info">
        ...
        <div class="details-grid">
            {{-- Puedes reemplazar "Salon Chic" con tu nombre de empresa o una variable si es dinámico --}}
    ...
    <div class="table-responsive">
        <table class="table invoice-table">
            ...
            <tr>
                <th>CONCEPTO</th>
                <th>CANTIDAD</th>
                <th>PRECIO UNITARIO</th> {{-- Cambiado de PRECIO COMPRA --}}
                <th>TOTAL</th>
            </tr>
            </thead>
            <tbody>
            {{-- Bucle de datos cambiado a $venta->detalles --}}
            @forelse($venta->detalles as $detalle)
                <tr>
                    <td>{{ $detalle->producto->nombre }}</td>
                    <td>{{ $detalle->cantidad }}</td>
                    <td>L {{ number_format($detalle->precio_unitario, 2) }}</td> {{-- Manteniendo precio_unitario --}}
                    <td>L {{ number_format($detalle->subtotal, 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="4" class="text-center text-muted">No hay productos en esta venta.</td> {{-- Colspan ajustado --}}
                </tr>
            @endforelse
            </tbody>
            <tfoot>
            <tr>
                <th colspan="3" class="text-end">SUBTOTAL</th> {{-- Colspan ajustado --}}
    ...
    <div class="totals-container">
        <table class="table totals-table">
            ...
            <tr>
                <th>ISV (15%)</th> {{-- Cambiado de IVA (21%) --}}
                <td>L {{ number_format($venta->isv_15, 2) }}</td> {{-- Usando el campo isv_15 del modelo Venta --}}
            </tr>
            <tr>
                <th>EXONERADO</th>
                <td>L {{ number_format($venta->importe_exonerado ?? 0, 2) }}</td> {{-- Usando el campo importe_exonerado --}}
            </tr>
            <tr>
                <th>EXENTO</th>
                <td>L {{ number_format($venta->importe_exento ?? 0, 2) }}</td> {{-- Usando el campo importe_exento --}}
            </tr>
            <tr>
                <th>TOTAL VENTA</th> {{-- Cambiado de TOTAL COMPRA --}}
