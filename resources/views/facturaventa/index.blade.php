...
<head>
    ...
    <style>
        ...
        .beauty-header { text-align: center; margin-bottom: 2rem; position: relative; }
        .beauty-header h2 { color: #7B2A8D; font-weight: 700; font-size: 1.8rem; margin: 0; text-shadow: 0 2px 4px rgba(123, 42, 141, 0.1); }
        .beauty-header::after { content: ''; display: block; width: 120px; height: 3px; background: linear-gradient(90deg, #E4007C, #7B2A8D); margin: 15px auto; border-radius: 2px; }
        ...
        .form-label { font-weight: 600; color: #7B2A8D; margin-bottom: 0.5rem; }

        .table-container { border-radius: 15px; overflow: hidden; box-shadow: 0 10px 25px rgba(123, 42, 141, 0.1); margin-bottom: 1rem; }
        .table { table-layout: fixed; width: 100%; word-wrap: break-word; }
        .table-responsive { overflow-x: visible !important; }
        .table td, .table th { word-break: break-word; white-space: normal !important; }
        .table thead th {
        ...
        .table tbody tr:hover { background-color: rgba(228, 0, 124, 0.05); transform: scale(1.01); }
        ...
        .btn-primary-beauty { background: linear-gradient(135deg, #E4007C 0%, #7B2A8D 100%); color: white; }
        .btn-primary-beauty:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(228,0,124,0.4); color:white; }

        .btn-secondary-beauty { background: linear-gradient(135deg, #7b2a8d 0%, #ff69b4 100%); color:white; }
        .btn-secondary-beauty:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgb(123,42,141); color:white; }

        .btn-danger-beauty { background: linear-gradient(135deg, #dc3545 0%, #a42a3a 100%); color:white; }
        .btn-danger-beauty:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(220,53,69,0.4); color:white; }

        .action-buttons { display: flex; gap: 0.5rem; justify-content: center; flex-wrap: wrap; }

        .alert { border-radius: 15px; border: none; box-shadow: 0 8px 25px rgba(40,167,69,0.15); animation: slideInDown 0.5s ease-out; }
        .alert-success { background: linear-gradient(135deg, rgba(40,167,69,0.1), rgba(25,135,84,0.05)); border-left: 4px solid #28a745; }

        #mensajeResultados { background: rgba(123,42,141,0.05); border:2px solid rgba(228,0,124,0.1); border-radius:15px; padding:1rem; margin-top:1rem; font-weight:600; color:#7B2A8D; text-align:center; }
...
<div class="container-fluid py-5">
    ...
    <div class="container py-5">
        ...
        <div class="header-actions">
            <form id="filtroVentas" method="GET" action="{{ route('facturaventa.index') }}" class="row g-3 align-items-end">
                <div class="col-md-5">
                    <label for="cliente" class="form-label"><i class="fas fa-user-tag"></i> Cliente</label>
                    <input type="text" id="cliente" name="cliente" class="form-control"
                           placeholder="Nombre cliente..." value="{{ request('cliente') }}">
                </div>

                <div class="col-md-5">
                    <label for="fecha_rango" class="form-label"><i class="fas fa-calendar-alt"></i> Rango de Fechas</label>
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle mb-0">
                    ...
                    @forelse ($ventas as $index => $venta)
                        <tr>
                            <td>{{ $index + 1 }}</td> {{-- Esto muestra 1, 2, 3… en lugar del id --}}
    ...
    <script>
        const form = document.getElementById('filtroVentas');
        let timeout = null;

        function submitForm() { form.submit(); }
        function debounceSubmit() { clearTimeout(timeout); timeout = setTimeout(submitForm, 700); }
        ...
        const fp = flatpickr(fechaRangoInput, {
            ...
            maxDate: "today",
            minDate: "2000-01-01",
            onClose: function(selectedDates, dateStr, instance) { submitForm(); }
        });
